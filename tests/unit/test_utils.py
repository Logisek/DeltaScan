...
class TestUtils(unittest.TestCase):
    ...
    def test_dict_diff_handler(self):
        result = _dict_diff_handler(DIFFS[0]["diffs"], [], "added")
        self.assertEqual(result, ARTICULATED_DIFFS[0]["added"])

        result = _dict_diff_handler(DIFFS[0]["diffs"], [], "changed")
        self.assertEqual(result, ARTICULATED_DIFFS[0]["changed"])

        result = _dict_diff_handler(DIFFS[0]["diffs"], [], "removed")
        self.assertEqual(result, ARTICULATED_DIFFS[0]["removed"])
        
        result = _dict_diff_handler(DIFFS[1]["diffs"], [], "added")
        self.assertEqual(result, ARTICULATED_DIFFS[1]["added"])

        result = _dict_diff_handler(DIFFS[1]["diffs"], [], "changed")
        self.assertEqual(result, ARTICULATED_DIFFS[1]["changed"])

        result = _dict_diff_handler(DIFFS[1]["diffs"], [], "removed")
